...
export default function ClientsPage() {
  ...
  async function addClient() {
    ...
    setForm({ store_name: '', email: '', phone: '', status: 'active', subscription_end: '', notes: '' })
    setShowAdd(false)
    loadClients()
  }
  ...
  function startEdit(c: Client) {
    setEditClient(c)
    setForm({
      store_name: c.store_name, email: c.email, phone: c.phone || '',
  ...
  const FormPanel = () => (
    <div className="card" style={{ border: '2px solid #185FA5' }}>
      <h3 style={{ fontSize: 15, fontWeight: 600, marginBottom: 16 }}>
        {editClient ? '✏️ Клиентті өзгерту' : '➕ Жаңа клиент қосу'}
      </h3>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
        <input placeholder="Дүкен аты *" value={form.store_name} onChange={e => setForm({ ...form, store_name: e.target.value })} />
        ...
        <input placeholder="Ескертпе" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} />
      </div>
      <div style={{ display: 'flex', gap: 8 }}>
        <button className="btn btn-primary" onClick={editClient ? updateClient : addClient}>
          {editClient ? 'Сақтау' : 'Қосу'}
        </button>
        <button className="btn" onClick={() => { setShowAdd(false); setEditClient(null) }}>Болдырмау</button>
      </div>
    </div>
  )
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        <button className="btn btn-primary" onClick={() => { setShowAdd(!showAdd); setEditClient(null) }}>
          + Клиент қосу
        </button>
      </div>

      {(showAdd && !editClient) && <FormPanel />}
      {editClient && <FormPanel />}
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <table className="table">
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: 'center', color: '#6b7280', padding: 24 }}>Клиент жоқ</td></tr>
            ) : filtered.map(c => (
              <tr key={c.id}>
                ...
                <td>
                  {c.subscription_end ? (
                    <span style={{ color: isExpired(c.subscription_end) ? '#D85A30' : isExpiringSoon(c.subscription_end) ? '#D97706' : '#0F6E56', fontSize: 13 }}>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button className={`btn btn-sm ${c.status === 'active' ? 'btn-warning' : 'btn-success'}`}
